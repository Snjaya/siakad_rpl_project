                ...
                <div class="p-8">
                    ...
                    <form action="{{ route('tu.classrooms.store') }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="col-span-1 md:col-span-2">
                                <label class="block text-sm font-medium text-gray-700 mb-1">Wali Kelas
                                    (Opsional)</label>
                                <select name="nip_teacher"
                                    ...
                                    @foreach ($teachers as $teacher)
                                        <option value="{{ $teacher->nip }}"
                                            {{ old('nip_teacher') == $teacher->nip ? 'selected' : '' }}>
                                            {{ $teacher->nama_guru }} (NIP: {{ $teacher->nip }})
                                        </option>
                                    @endforeach
                                </select>
                                <p class="text-xs text-gray-400 mt-1">Dapat dikosongkan jika belum ada wali kelas.</p>
                                @error('nip_teacher')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
